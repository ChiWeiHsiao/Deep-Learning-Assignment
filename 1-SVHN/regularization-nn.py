...
class NeuralNetwork():
    def __init__(self, train_x, train_label, test_x, test_label, h_nodes1, h_nodes2, batch_size=20, learning_rate=0.05, learn_decay='False', n_hidden_layers=2, epoches=100, regularization=0.1, outfile='try.txt'):
        # Data
        self.train_x, self.train_label =  train_x, train_label # Xs: a list of 45000 * np_arrays with shape=(1,784)
        self.test_x, self.test_label =  test_x, test_label # Xs: a list of 45000 * np_arrays with shape=(1,784)
        # Parameters
        self.nh1 = h_nodes1
        self.nh2 = h_nodes2
        self.ni = self.train_x[0].shape[0] #number of input nodes, 784
        self.no = self.train_label[0].shape[0] #number of output nodes, 10
        ...
        print(outfile)
        print('Regularization lambda: '+str(regularization))
        ...
        with open(self.filename, 'w') as fp:
            fp.write('Regularization lambda: '+str(regularization)+'\n')
            ...
            fp.write('initial learning rate: '+str(learning_rate)+'\n')
        ...
        for epoche in range(0, epoches):
            ...
            for batch in range(0, last_batch, batch_size):
                ...
                for p in range(start_pattern, end_pattern):
                    #forward propagation
                    A = self.forward_propagate(self.train_x[p])
                    delta = self.get_shape_of_node()    # delta[l][i] = (10,) #array
                    delta[self.layers-1] = A[self.layers-1] - self.train_label[p] #softmax derivative = y-t
                ...
                for l in range(self.layers-1):
                    D = (Gradient[l] + self.regularization * self.weight[l]) / batch_size
            ...
            train_error_this_epoche = self.error_rate(self.train_x, self.train_label)
            test_error_this_epoche = self.error_rate(self.test_x, self.test_label)
    ...
    def error_rate(self, inputs, targets):
        ...
        for i in range(m):
            ...
            cost += self.cost_function(targets[i], output)
        rate = {'miss': miss/m, 'cost': cost/m}
        return  rate

    def cost_function(self, Target, Output):
        # Cross Entropy of a single example
        # Output: probability vector
        cross_entropy = np.sum(-(Target*np.log(Output)))
        regularization_term = 0
        for l in range(self.layers-1):
            regularization_term += np.sum(self.weight[l]**2)
        regularization_term *= self.regularization / 2
        return cross_entropy + regularization_term

    def shuffle_data(self):
        # shuffle before each epoche
        pairs = list(zip(self.train_x, self.train_label))
        random.shuffle(pairs)
        self.train_x, self.train_label = zip(*pairs)
...
if __name__ == "__main__":
    ...
    #training
    filename = 'result.txt'
    node1, node2, l = 200, 100, 2
    batch, e = 20, 60
    rate , learn_decay = 0.3, False
    regular = 0.03

    nn = NeuralNetwork(train_x, train_label, test_x, test_label, h_nodes1=node1, h_nodes2=node2, batch_size=batch, learning_rate=rate, learn_decay=learn_decay,  n_hidden_layers=l, epoches=e, regularization=regular, outfile=filename)
    nn.train()
